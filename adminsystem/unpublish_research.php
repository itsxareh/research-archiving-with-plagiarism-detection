...
if(isset($_GET['archiveID'])){
    ...
    if ($select_research_owner_email){
        ...
        $sql2 = $db->adminsystem_INSERT_NOTIFICATION($admin_id, "You have unpublished a research paper.", $date, $time);
        $sql3 = $db->student_INSERT_NOTIFICATION($select_research_owner_email['studentID'], "Your " . $select_research_owner_email['project_title'] . " research paper has been unpublished.", $date, $time);
        ...
        $mail->Body = 'Your ' . $select_research_owner_email['project_title'] . ' research paper has been unpublished.';
    
        if($mail->send()){
            echo json_encode(array("status_code" => "success", "status" => "Research unpublished", "alert" => "Success"));
        } else {
