...
if(ISSET($_POST['add_department'])){
    ...
    $validate_department_code = $db->SELECT_DEPARTMENT_BY_CODE($department_code, '');
    $validate_department_name = $db->SELECT_DEPARTMENT_BY_NAME($department_name, '');

    if($validate_department_code || $validate_department_name){
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        <div class="item-detail">
                                            ...
                                            <input type="text" class="info-input" name="department_code" id="department_code" minlength="3" required>
                                            <span class="error-message" id="department_code_error" style="display: none;"></span>
                                        </div>
                                        <div class="item-detail">
                                            <label for="" class="info-label m-l-4">Department</label>
                                            <input type="text" class="info-input" name="department_name" id="department_name" minlength="3" required>
                                            <span class="error-message" id="department_name_error" style="display: none;"></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                    <button name="add_department" class="btn btn-danger" id="add_department_button">Save</button>
                                </div>
                            </form>
                                        ...
                                        <div class="dropdown-action" id="dropdown_<?= $result['id'] ?>" role="action" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                            <div role="none">
                                                <?php if (hasPermission($permissions, 'department_edit')): ?>   
                                                    <a href="#" data-toggle="modal" data-target="#modelId_<?= $result['id'] ?>" class="dropdown-action-item">Edit</a>
                                                <?php endif; ?>
                                                <?php if (hasPermission($permissions, 'department_delete')): ?>
                                                    <a onclick="confirmDelete(<?= $result['id'] ?>)" href="#" data-toggle="delete-modal" data-target="#delete_modelId_<?= $result['id'] ?>" class="dropdown-action-item">Delete</a>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <form action="" method="post">
                                                <div class="modal-body">
                                                    ...
                                                    <div class="form-group">
                                                        <div class="item-detail">
                                                            <label for="" class="info-label m-l-4">Acronym</label>
                                                            <input type="text" class="info-input edit-department-code" 
                                                                name="dept_code" 
                                                                value="<?= $result['dept_code'] ?>" 
                                                                data-original="<?= $result['dept_code'] ?>"
                                                                data-id="<?= $result['id'] ?>"
                                                                minlength="3" 
                                                                required>
                                                            <span class="error-message" id="edit_department_code_error_<?= $result['id'] ?>" style="display: none;"></span>
                                                        </div>
                                                        <div class="item-detail">
                                                            <label for="" class="info-label m-l-4">Department</label>
                                                            <input type="text" class="info-input edit-department-name" 
                                                                name="dept_name" 
                                                                value="<?= $result['name'] ?>" 
                                                                data-original="<?= $result['name'] ?>"
                                                                data-id="<?= $result['id'] ?>"
                                                                minlength="3" 
                                                                required>
                                                            <span class="error-message" id="edit_department_name_error_<?= $result['id'] ?>" style="display: none;"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                    <button name="edit" class="btn btn-danger edit-department-button" id="edit_department_button_<?= $result['id'] ?>">Update</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
...
$("#department_code").on("input", function() {
    validateDepartment("code");
}); 
$("#department_name").on("input", function() {
    validateDepartment("name");
}); 

function validateDepartment(type) {
    var department_code = $("#department_" + type).val();
    
    if (department_code.length < 3) {
        $("#department_" + type + "_error").text("Department " + type + " must be at least 3 characters long").show();
        return;
    }

    $.ajax({
        url: "validate_department_" + type + ".php",
        method: "POST",
        ...
        success: function(response) {
            try {
                ...
                if (data.exists) {
                    ...
                    $("#add_department_button").prop("disabled", true);
                } else {
                    $("#department_" + type + "_error").hide();
                    $("#department_" + type).css('background-image', 'url("../images/checked.png")');
                    $("#add_department_button").prop("disabled", false);
                }
            } catch (e) {
...
$(".edit-department-code").on("input", function() {
    const id = $(this).data('id');
    const originalValue = $(this).data('original');
    validateEditDepartment("code", id, originalValue);
}); 

$(".edit-department-name").on("input", function() {
    const id = $(this).data('id');
    const originalValue = $(this).data('original');
    validateEditDepartment("name", id, originalValue);
}); 

function validateEditDepartment(type, id, originalValue) {
    var department_value = $(`input[data-id='${id}'].edit-department-${type}`).val();
    
    // If value hasn't changed from original, clear errors and enable button
    if (department_value === originalValue) {
        $(`#edit_department_${type}_error_${id}`).hide();
        $(`input[data-id='${id}'].edit-department-${type}`).css('background-image', '');
        $(`#edit_department_button_${id}`).prop("disabled", false);
        return;
    }

    // Check minimum length
    if (department_value.length < 3) {
        $(`#edit_department_${type}_error_${id}`).text(`Department ${type} must be at least 3 characters long`).show();
        $(`#edit_department_button_${id}`).prop("disabled", true);
        return;
    }
    ...
    $.ajax({
        ...
        method: "POST",
        data: { 
            department_code: department_value,
            exclude_id: id
        },
        success: function(response) {
            try {
                const data = JSON.parse(response);
                if (data.exists) {
                    $(`#edit_department_${type}_error_${id}`).text(`Department ${type} already exists`).show();
                    $(`input[data-id='${id}'].edit-department-${type}`).css('background-image', 'url("../images/close.png")');
                    $(`#edit_department_button_${id}`).prop("disabled", true);
                } else {
                    $(`#edit_department_${type}_error_${id}`).hide();
                    $(`input[data-id='${id}'].edit-department-${type}`).css('background-image', 'url("../images/checked.png")');
                    $(`#edit_department_button_${id}`).prop("disabled", false);
                }
            } catch (e) {
                console.error("Error parsing response:", e);
