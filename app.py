#https://github.com/oschwartz10612/poppler-windows/releases/
#https://github.com/UB-Mannheim/tesseract/wiki
#https://github.com/tesseract-ocr/tessdata
import os
import re
from typing import Dict, List, Tuple, Any
from flask import Flask, request, jsonify, send_from_directory, Response
from PyPDF2 import PdfReader
from docx import Document
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity
import mysql.connector
from mysql.connector import Error
import nltk
from nltk.tokenize import sent_tokenize, word_tokenize
from nltk.corpus import stopwords
from nltk.stem import WordNetLemmatizer
from datetime import datetime
from threading import Thread
import pytesseract
import time
import tempfile
import shutil
import io
import fitz  # PyMuPDF
import pytz
import numpy as np
import string
import gc 
from werkzeug.utils import secure_filename

nltk.download('punkt', quiet=True)
nltk.download('stopwords', quiet=True)
nltk.download('wordnet', quiet=True)
nltk.download('averaged_perceptron_tagger', quiet=True)

# Configure tesseract for Windows
if os.name == 'nt':  # for Windows
    POPPLER_PATH = r"C:\Program Files\poppler-24.08.0\Library\bin"
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
try:
    print(f"Tesseract Version: {pytesseract.get_tesseract_version()}")
    print(f"Available languages: {pytesseract.get_languages()}")
except Exception as e:
    print(f"Tesseract Error: {str(e)}")
    print("Please ensure Tesseract is properly installed and the path is correct.")

from PIL import Image, ImageEnhance
from pdf2image import convert_from_path
import cv2

app = Flask(__name__)
# Increase maximum content length to 500MB
app.config['MAX_CONTENT_LENGTH'] = 500 * 1024 * 1024

# Create folders for temporary and uploaded files
TEMP_FOLDER = os.path.join(os.getcwd(), 'temp_files')
UPLOAD_FOLDER = os.path.join(os.getcwd(), 'pdf_files')

# Create directories if they don't exist
os.makedirs(TEMP_FOLDER, exist_ok=True)
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# Status tracking for large file uploads
upload_status = {}

def get_db():
    try:
        conn = mysql.connector.connect(
            host='localhost',
            user='root', 
            password='', 
            database='research_repository',
            connect_timeout= 300  # Increase timeout for large operations
        )
        if conn.is_connected():
            return conn
    except Error as e:
        print(f"Error connecting to MySQL: {e}")
        return None
    
def init_db():
    with get_db() as conn:
        cursor = conn.cursor()
        
        # Modified archive_research table with status field
        cursor.execute('''CREATE TABLE IF NOT EXISTS archive_research (
            id INT AUTO_INCREMENT PRIMARY KEY,
            archive_id INT NOT NULL,
            student_id VARCHAR(20) NOT NULL,
            department_id INT NOT NULL,
            course_id INT NOT NULL,
            project_title VARCHAR(255) NOT NULL,
            dateOfSubmit DATETIME NOT NULL,
            project_year VARCHAR(4) NOT NULL,
            project_abstract TEXT,
            keywords TEXT,
            content TEXT NOT NULL,
            research_owner_email VARCHAR(255),
            project_members TEXT,
            documents VARCHAR(255),
            file_size BIGINT UNSIGNED,
            page_count INT UNSIGNED,
            word_count INT UNSIGNED,
            character_count INT UNSIGNED,
            document_status VARCHAR(50) DEFAULT 'Processing',
            date_published DATE NULL
        )''')

        cursor.execute('''CREATE TABLE IF NOT EXISTS plagiarism_results (
            id INT AUTO_INCREMENT PRIMARY KEY,
            archive_id INT NOT NULL,
            submitted_sentence TEXT NOT NULL,
            existing_sentence TEXT NOT NULL,
            similar_archive_id INT NOT NULL,
            similarity_percentage DECIMAL(5,2) NOT NULL,
            is_plagiarized BOOLEAN NOT NULL,
            FOREIGN KEY (archive_id) REFERENCES archive_research(id),
            FOREIGN KEY (similar_archive_id) REFERENCES archive_research(id)
        )''')

        cursor.execute('''CREATE TABLE IF NOT EXISTS plagiarism_summary (
            id INT AUTO_INCREMENT PRIMARY KEY,
            archive_id INT NOT NULL,
            similar_archive_id INT NOT NULL,
            plagiarism_percentage DECIMAL(5,2) NOT NULL,
            FOREIGN KEY (archive_id) REFERENCES archive_research(id),
            FOREIGN KEY (similar_archive_id) REFERENCES archive_research(id)
        )''')
        
        conn.commit()

SUPPORTED_LANGUAGES = {
    'eng': 'English',
    'fil': 'Filipino/Tagalog',
    'ceb': 'Cebuano',
    'ilo': 'Ilocano',
    'hil': 'Hiligaynon',
    'bik': 'Bikol',
    'war': 'Waray',
    'pam': 'Kapampangan',
    'pan': 'Pangasinan'
}

def detect_language(text):
    """
    Attempt to detect the language of the text to choose appropriate OCR language
    """
    # Common Filipino words/markers
    filipino_markers = [
        'ng', 'mga', 'na', 'sa', 'ang', 'ay', 'at', 'po', 'ko', 'ka',
        'namin', 'natin', 'kami', 'tayo', 'siya', 'niya', 'nila', 'ninyo',
        'ako', 'bakit', 'ano', 'saan', 'paano', 'kailan'
    ]
    
    text_lower = text.lower()
    filipino_word_count = sum(1 for word in filipino_markers if word in text_lower.split())
    
    # If text contains significant Filipino markers, include Filipino in OCR
    if filipino_word_count > 2:
        return ['eng', 'fil']  # Include both English and Filipino
    return ['eng']  # Default to English

def preprocess_image(image):
    try:
        # Convert PIL Image to OpenCV format if necessary
        if isinstance(image, Image.Image):
            image_cv = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        else:
            image_cv = image

        # Step 1: Convert to grayscale
        gray = cv2.cvtColor(image_cv, cv2.COLOR_BGR2GRAY)

        # Step 2: Apply CLAHE for contrast enhancement
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        contrast_enhanced = clahe.apply(gray)

        # Step 3: Increase image size
        scale_factor = 2.0  # Reduced from 2.5 for performance
        enlarged = cv2.resize(contrast_enhanced, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)

        # Step 4: Apply Gaussian blur and sharpening
        blurred = cv2.GaussianBlur(enlarged, (5, 5), 0)
        sharpened = cv2.addWeighted(enlarged, 1.5, blurred, -0.5, 0)

        # Step 5: Apply adaptive thresholding
        binary = cv2.adaptiveThreshold(
            sharpened,
            255,
            cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
            cv2.THRESH_BINARY,
            15, 10
        )

        # Step 6: Denoise
        denoised = cv2.fastNlMeansDenoising(binary, None, 30, 7, 21)
        
        # Step 7: Convert back to PIL Image
        enhanced_image = Image.fromarray(denoised)
        
        return enhanced_image

    except Exception as e:
        print(f"Image preprocessing error: {str(e)}")
        return image

def extract_text_from_image(image, default_languages=['eng', 'fil']):
    try:
        # Preprocess the image
        processed_image = preprocess_image(image)
        
        # Get initial OCR text
        custom_config = r'--oem 3 --psm 6 --dpi 300'
        text = pytesseract.image_to_string(
            processed_image,
            lang='+'.join(default_languages),
            config=custom_config
        )
        
        # Clean up text
        text = text.strip()
        text = re.sub(r'\s+', ' ', text)
        
        confidence = min(1.0, len(text) / 100) if text else 0.0
        return text, confidence
        
    except Exception as e:
        print(f"OCR Error: {str(e)}")
        return "", 0

def process_pdf_in_chunks(file_path, archive_id):
    """Process a large PDF file in chunks to avoid memory issues"""
    try:
        upload_status[archive_id] = {
            'status': 'Processing PDF',
            'progress': 0,
            'total_pages': 0,
            'processed_pages': 0
        }
        
        # Use PyMuPDF (fitz) for efficient PDF handling
        pdf_document = fitz.open(file_path)
        total_pages = len(pdf_document)
        
        upload_status[archive_id]['total_pages'] = total_pages
        
        # Process in chunks of 10 pages
        chunk_size = 10
        all_text = ""
        
        for start_page in range(0, total_pages, chunk_size):
            end_page = min(start_page + chunk_size, total_pages)
            chunk_text = ""
            
            for page_num in range(start_page, end_page):
                page = pdf_document[page_num]
                
                # Extract text directly from PDF
                page_text = page.get_text()
                chunk_text += page_text
                
                # Process a limited number of images to avoid memory issues
                image_list = page.get_images(full=True)
                for img_index, img in enumerate(image_list[:5]):  # Process max 5 images per page
                    try:
                        xref = img[0]
                        base_image = pdf_document.extract_image(xref)
                        image_bytes = base_image["image"]
                        image = Image.open(io.BytesIO(image_bytes))
                        
                        # Resize large images to conserve memory
                        if image.width > 1000 or image.height > 1000:
                            image.thumbnail((1000, 1000), Image.LANCZOS)
                            
                        detected_langs = detect_language(page_text)
                        text, score = extract_text_from_image(image, detected_langs)
                        if text and text.strip():
                            chunk_text += f"\n{text}\n"
                    except Exception as e:
                        print(f"Error processing PDF embedded image {img_index} on page {page_num}: {str(e)}")
                
                # Update progress
                upload_status[archive_id]['processed_pages'] = page_num + 1
                upload_status[archive_id]['progress'] = int((page_num + 1) / total_pages * 100)
                
                # Force garbage collection to free memory
                gc.collect()
            
            all_text += chunk_text
            
            # Sleep briefly to allow other processes to run
            time.sleep(0.1)
        
        pdf_document.close()
        
        # Calculate metrics
        metrics = {
            'page_count': total_pages,
            'word_count': len(all_text.split()),
            'character_count': len(all_text)
        }
        
        upload_status[archive_id]['status'] = 'PDF Processing Complete'
        upload_status[archive_id]['progress'] = 100
        
        return metrics, all_text
        
    except Exception as e:
        upload_status[archive_id]['status'] = f'Error: {str(e)}'
        print(f"Error processing PDF in chunks: {str(e)}")
        return {'page_count': 0, 'word_count': 0, 'character_count': 0}, ""

def get_file_metrics(file_path, file_type, archive_id):
    """Calculate file metrics including size, page count, word count, and character count"""
    metrics = {
        'file_size': os.path.getsize(file_path),
        'page_count': 0,
        'word_count': 0,
        'character_count': 0
    }
    
    content = ""
    
    # Default languages including Filipino
    default_languages = ['eng', 'fil']
    
    if file_type == 'application/pdf':
        # Use the chunked processing method for PDFs
        pdf_metrics, content = process_pdf_in_chunks(file_path, archive_id)
        metrics.update(pdf_metrics)
            
    elif file_type in ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/msword']:
        upload_status[archive_id] = {
            'status': 'Processing Word Document',
            'progress': 0
        }
        
        doc = Document(file_path)
        metrics['page_count'] = len(doc.paragraphs) // 40  # Approximate pages
        
        for i, para in enumerate(doc.paragraphs):
            content += para.text + "\n"
            if i % 20 == 0:  # Update progress every 20 paragraphs
                upload_status[archive_id]['progress'] = min(90, int((i / len(doc.paragraphs)) * 100))
        
        # Extract images from Word document (limit to 20 images)
        image_count = 0
        for rel in doc.part.rels.values():
            if "image" in rel.target_ref and image_count < 20:
                try:
                    image_data = rel.target_part.blob
                    image = Image.open(io.BytesIO(image_data))
                    # Resize large images
                    if image.width > 1000 or image.height > 1000:
                        image.thumbnail((1000, 1000), Image.LANCZOS)
                    image_text, score = extract_text_from_image(image, default_languages)
                    if image_text and image_text.strip():
                        content += f"\n{image_text}\n"
                    image_count += 1
                except Exception as e:
                    print(f"Error processing image in Word document: {str(e)}")
        
        upload_status[archive_id]['status'] = 'Word Processing Complete'
        upload_status[archive_id]['progress'] = 100
    
    else:  # For text files
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                content = file.read()
                metrics['page_count'] = len(content) // 3000  # Approximate pages
        except UnicodeDecodeError:
            # Try with a different encoding if UTF-8 fails
            try:
                with open(file_path, 'r', encoding='latin-1') as file:
                    content = file.read()
                    metrics['page_count'] = len(content) // 3000
            except Exception as e:
                print(f"Error reading text file: {str(e)}")
                content = ""
    
    # Calculate word and character counts
    metrics['word_count'] = len(content.split())
    metrics['character_count'] = len(content)
    
    return metrics, content

def normalize_text(text):
    # Convert to lowercase
    text = text.lower()

    # Remove leading and trailing spaces
    text = text.strip()
    
    # Replace unicode punctuation with standard punctuation
    text = text.translate(str.maketrans('', '', string.punctuation))
    
    # Remove multiple spaces between words
    text = re.sub(r'\s+', ' ', text)
    
    # Normalize hyphenated words (removes spaces around hyphens)    
    text = re.sub(r'\s*-\s*', '-', text)
    
    # Remove non-alphanumeric characters except spaces and hyphens
    text = re.sub(r'[^\w\s-]', '', text)

    # Handle redundant punctuation like multiple dots
    text = re.sub(r'\.\.+', '.', text)

    # Replace multiple consecutive hyphens with a single hyphen
    text = re.sub(r'-{2,}', '-', text)
    
    return text

def split_into_sentences(text):
    try:
        nltk.data.find('tokenizers/punkt')
    except LookupError:
        nltk.download('punkt')
    
    # First, preserve certain abbreviations and numbers
    preserved_text = text
    preserved_text = re.sub(r'(?<=\d)\.(?=\d)', '@DOT@', preserved_text)  # Preserve decimal points
    preserved_text = re.sub(r'(?<=\w)\.(?=\w)', '@DOT@', preserved_text)  # Preserve abbreviations
    
    # Split into sentences
    sentences = sent_tokenize(preserved_text)
    
    # Restore preserved characters
    sentences = [s.replace('@DOT@', '.') for s in sentences]
    
    # Additional cleaning and filtering
    cleaned_sentences = []
    for sentence in sentences:
        # Normalize the sentence
        sentence = normalize_text(sentence)
        
        # Count actual words (excluding numbers and single characters)
        word_count = len([w for w in sentence.split() if re.match(r'[a-zA-Z]{2,}', w)])
        
        # Filter sentences based on word count and other criteria
        if (7 <= word_count <= 70 and  # Word count between 7 and 70
            len(sentence) >= 40 and     # Minimum character length
            not re.match(r'^[^a-zA-Z]*$', sentence) and  # Contains letters
            not re.match(r'^(table|figure|fig)', sentence.lower())):  # Not a table/figure reference
            cleaned_sentences.append(sentence)
    
    return cleaned_sentences

def calculate_similarity_in_chunks(submitted_content, submissions, archive_id, similarity_threshold=0.7):
    """Calculate similarity in chunks to handle memory constraints"""
    upload_status[archive_id]['status'] = 'Checking for plagiarism'
    upload_status[archive_id]['progress'] = 0
    
    submitted_sentences = [normalize_text(sentence) for sentence in split_into_sentences(submitted_content)]
    
    if len(submitted_sentences) < 5:
        upload_status[archive_id]['status'] = 'Complete - Too few sentences to check'
        upload_status[archive_id]['progress'] = 100
        return []
    
    chunk_size = 100
    all_results = []
    total_submissions = len(submissions)
    
    for sub_idx, (submission_id, content) in enumerate(submissions):
        existing_sentences = [normalize_text(sentence) for sentence in split_into_sentences(content)]
        
        for start_idx in range(0, len(submitted_sentences), chunk_size):
            end_idx = min(start_idx + chunk_size, len(submitted_sentences))
            submitted_chunk = submitted_sentences[start_idx:end_idx]
            
            all_sentences = submitted_chunk + existing_sentences
            
            if len(all_sentences) < 2: 
                continue
                
            try:
                vectorizer = TfidfVectorizer(lowercase=True, stop_words=None, token_pattern=r'\b\w+\b')
                vectors = vectorizer.fit_transform(all_sentences)
                
                chunk_size_actual = end_idx - start_idx
                similarity_matrix = cosine_similarity(
                    vectors[:chunk_size_actual], 
                    vectors[chunk_size_actual:]
                )
                
                for i, submitted_idx in enumerate(range(start_idx, end_idx)):
                    for j, existing_idx in enumerate(range(len(existing_sentences))):
                        similarity_score = similarity_matrix[i][j]
                        if similarity_score > similarity_threshold:
                            all_results.append({
                                'submitted_sentence': submitted_sentences[submitted_idx],
                                'existing_sentence': existing_sentences[existing_idx],
                                'similarity_percentage': similarity_score * 100,
                                'similar_archive_id': submission_id
                            })
            except Exception as e:
                print(f"Error during similarity calculation: {str(e)}")
                continue
                
            gc.collect()
            
        progress = int((sub_idx + 1) / total_submissions * 100)
        upload_status[archive_id]['progress'] = progress
    
    upload_status[archive_id]['status'] = 'Plagiarism check complete'
    upload_status[archive_id]['progress'] = 100
    
    return all_results

@app.route('/')
def index():    
    return send_from_directory('.', 'student/project_list.php')

@app.route('/upload_status/<archive_id>', methods=['GET'])
def get_upload_status(archive_id):
    """Endpoint to check the status of a file upload and processing"""
    status = upload_status.get(archive_id, {'status': 'Not found', 'progress': 0})
    return jsonify(status)

@app.route('/upload_research', methods=['POST'])
def upload_file():
    print("Form Data:", request.form)
    print("Files Data:", request.files)
    if 'file' not in request.files:
        return jsonify({'status': 'error', 'message': "No file part"}), 400

    file = request.files['file']
    if file.filename == '':
        return jsonify({'status': 'error', 'message': "No selected file"}), 400

    # Extract form data
    archive_id = request.form.get('archive_id')
    student_id = request.form.get('student_id')
    project_title = request.form.get('project_title')
    project_year = request.form.get('year')
    department_id = request.form.get('department_id')
    course_id = request.form.get('course_id')
    abstract = request.form.get('abstract')
    keywords = request.form.get('keywords')
    project_members = request.form.get('project_members')
    pdf_path = request.form.get('pdf_path', '')
    owner_email = request.form.get('owner_email')

    if not student_id:
        return jsonify({'status': 'error', 'message': "No student ID provided"}), 400

    try:
        upload_status[archive_id] = {
            'status': 'Uploading file',
            'progress': 0
        }
        
        temp_dir = os.path.join(TEMP_FOLDER, archive_id)
        os.makedirs(temp_dir, exist_ok=True)
        
        secure_name = secure_filename(file.filename)
        temp_file_path = os.path.join(temp_dir, secure_name)
        file.save(temp_file_path)
        
        upload_status[archive_id]['status'] = 'File saved, processing content'
        upload_status[archive_id]['progress'] = 20
        
        file_metrics, content = get_file_metrics(temp_file_path, file.content_type, archive_id)
        
        if not content:
            return jsonify({'status': 'error', 'message': "Failed to extract content from file"}), 400
        
        filename_base = f"{archive_id}_{int(time.time())}"
        file_ext = os.path.splitext(secure_name)[1]
        final_filename = f"{filename_base}{file_ext}"
        final_path = os.path.join(UPLOAD_FOLDER, final_filename)
        
        final_destination = pdf_path if pdf_path and os.path.isdir(os.path.dirname(pdf_path)) else final_path
        
        os.makedirs(os.path.dirname(final_destination), exist_ok=True)
        
        manila_tz = pytz.timezone('Asia/Manila')
        current_time = datetime.now(manila_tz)
        
        upload_status[archive_id]['status'] = 'Saving to database'
        upload_status[archive_id]['progress'] = 60
        
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO archive_research (
                    archive_id, student_id, department_id, course_id,
                    project_title, dateOfSubmit, project_year, project_abstract,
                    keywords, content, research_owner_email, project_members,
                    documents, file_size, page_count, word_count, character_count,
                    document_status
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (
                    archive_id, student_id, department_id, course_id,
                    project_title, current_time, project_year, abstract,
                    keywords, content, owner_email, project_members,
                    final_destination, file_metrics['file_size'], file_metrics['page_count'],
                    file_metrics['word_count'], file_metrics['character_count'],
                    'Processing'
                ))
            conn.commit()
            new_archive_id = cursor.lastrowid

            cursor.execute("SELECT * FROM departments WHERE id = %s", (department_id,))
            departments_db = cursor.fetchall()
            department = departments_db[0][2] if departments_db else "Unknown Department"

        upload_status[archive_id]['status'] = 'Finalizing upload'
        upload_status[archive_id]['progress'] = 80
        
        try:
            dest_dir = os.path.dirname(final_destination)
            if not os.path.exists(dest_dir):
                os.makedirs(dest_dir, exist_ok=True)
                
            shutil.copy2(temp_file_path, final_destination)
            print(f"File successfully copied to {final_destination}")
        except Exception as copy_error:
            print(f"Error copying file: {str(copy_error)}")
            shutil.copy2(temp_file_path, final_path)
            final_destination = final_path
            pdf_file_path = f"../pdf_files/{filename_base}{file_ext}"
            with get_db() as conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE archive_research SET documents = %s WHERE id = %s", 
                               (pdf_file_path, new_archive_id))
                conn.commit()
        
        Thread(target=plagiarism_check, args=(new_archive_id, student_id, content, current_time, archive_id)).start()

        response = {
            'status': 'success',
            'department': department,
            'document_status': 'Processing',
            'file_metrics': file_metrics,
            'archive_id': archive_id,
            'file_path': final_destination,
            'message': 'File uploaded successfully. Plagiarism check is running in the background.'
        }

        return jsonify(response)

    except Exception as e:
        print(f"Error: {str(e)}")
        upload_status[archive_id] = {
            'status': f'Error: {str(e)}',
            'progress': 0
        }
        return jsonify({'status': 'error', 'message': str(e)}), 500
    finally:
        if 'temp_file_path' in locals() and os.path.exists(temp_file_path):
            try:
                os.remove(temp_file_path)
            except:
                pass

def plagiarism_check(new_archive_id, student_id, content, current_time, archive_id):
    """Separate function for background plagiarism checking"""
    try:
        with get_db() as conn:
            cursor = conn.cursor()
        
            cursor.execute("""
                UPDATE archive_research 
                SET document_status = 'Checking Plagiarism' 
                WHERE id = %s
            """, (new_archive_id,))
            conn.commit()

            cursor.execute("SELECT id, content FROM archive_research WHERE student_id != %s", (student_id,))
            submissions = cursor.fetchall()

            plagiarism_results = {}
            if submissions:
                matching_sentences = calculate_similarity_in_chunks(content, submissions, archive_id)
                
                doc_matches = {}
                
                upload_status[archive_id]['status'] = 'Recording plagiarism results'
                
                for match in matching_sentences:
                    similar_id = match['similar_archive_id']
                    
                    if similar_id not in doc_matches:
                        doc_matches[similar_id] = set()
                    
                    # Add the matched sentence to track uniqueness
                    match_key = (match['submitted_sentence'], match['existing_sentence'])
                    doc_matches[similar_id].add(match_key)
                    
                    cursor.execute("""
                        INSERT INTO plagiarism_results (
                            archive_id, similar_archive_id, submitted_sentence,
                            existing_sentence, similarity_percentage, is_plagiarized
                        ) VALUES (%s, %s, %s, %s, %s, %s)
                        """, (
                        new_archive_id, similar_id, match['submitted_sentence'],
                        match['existing_sentence'], match['similarity_percentage'], True
                    ))
                    
                    if similar_id not in plagiarism_results:
                        cursor.execute("SELECT content FROM archive_research WHERE id = %s", (similar_id,))
                        similar_content = cursor.fetchone()[0]
                        total_sentences = len(split_into_sentences(similar_content))
                        
                        plagiarism_results[similar_id] = {
                            'matched_sentences': 0,
                            'total_similarity': 0,
                            'total_sentences': total_sentences
                        }
                    
                    # Only increment matched_sentences once per unique match
                    if len(doc_matches[similar_id]) > plagiarism_results[similar_id]['matched_sentences']:
                        plagiarism_results[similar_id]['matched_sentences'] = len(doc_matches[similar_id])
                    plagiarism_results[similar_id]['total_similarity'] += match['similarity_percentage']

                # Insert plagiarism summary records
                for similar_id, results in plagiarism_results.items():
                    avg_similarity = results['total_similarity'] / results['matched_sentences'] if results['matched_sentences'] > 0 else 0
                    plagiarism_percentage = (results['matched_sentences'] / results['total_sentences'] * 100) if results['total_sentences'] > 0 else 0

                    cursor.execute("""
                        INSERT INTO plagiarism_summary (
                            archive_id, similar_archive_id, plagiarism_percentage
                        ) VALUES (%s, %s, %s)
                        """, (new_archive_id, similar_id, plagiarism_percentage))
            
            conn.commit()

            # Calculate final plagiarism results and update status
            cursor.execute("""
                SELECT COUNT(plagiarism_summary.id) as total_ids, 
                       SUM(plagiarism_percentage) as total_percentage 
                FROM plagiarism_summary 
                WHERE archive_id = %s
                GROUP BY archive_id
                """, (new_archive_id,))
            
            result = cursor.fetchone()
            sum_of_percentage = result[1] if result else 0
            document_status = "Accepted" if sum_of_percentage <= 20 else "Rejected"
            date_publish = current_time.strftime('%Y-%m-%d') if document_status == "Accepted" else None

            cursor.execute("""
                UPDATE archive_research 
                SET document_status = %s, 
                    date_published = %s 
                WHERE id = %s
            """, (document_status, date_publish, new_archive_id))
            conn.commit()

    except Exception as e:
        print(f"Error during plagiarism check: {str(e)}")
        # Update status to indicate error
        try:
            with get_db() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    UPDATE archive_research 
                    SET document_status = %s
                    WHERE id = %s
                """, (f"Error: {str(e)[:100]}", new_archive_id))
                conn.commit()
        except:
            pass

@app.route('/check_db_connection', methods=['GET'])
def check_db_connection():
    try:
        with get_db() as conn:
            conn.execute("SELECT 1")
        return jsonify(status="Database connection successful"), 200
    except Exception as e:
        return jsonify(status="Database connection failed", error=str(e)), 500
    
if __name__ == '__main__':
    init_db()
    app.run(debug=True, port=3000)